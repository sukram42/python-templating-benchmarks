...
def bench_invoice(fixture: Fixture[Invoice]) -> BenchCase:
    ...
    def prepare() -> ashes_lib.AshesEnv:
        env = ashes_lib.AshesEnv()
        env.register_source("t", template_str)
        return env
    return BenchCase(prepare=prepare, render=lambda env: env.render("t", fixture.data.model_dump()))
...
def bench_deployment_report(fixture: Fixture[DeploymentReport]) -> BenchCase:
    ...
    def prepare() -> ashes_lib.AshesEnv:
        ...
        env.register_source("t", template_str)
        return env
    def render(env: ashes_lib.AshesEnv) -> str:
        d = fixture.data
        data = d.model_dump()
...
def bench_release_report(fixture):
    ...
    # Dust: {#section} iterates lists, {?key} is truthy check, dot notation for nested
    template_str = (
        "Release v{version} \u2014 {project}{?hotfix} [HOTFIX]{/hotfix}\n"
        "Team: {team.name} | Lead: {team.lead.name} ({team.lead.email})\n"
        "Components:\n"
        "{#components}"
        "- {name} [{?deployed}deployed{:else}not deployed{/deployed}]"
